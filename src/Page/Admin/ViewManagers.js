...
export default function ViewManagers(){
  ...
  async function fetchUser(){
    let res = await API.graphql(
      graphqlOperation(listUsers, { filter: { Identity: { eq: 'Manager' } }})
    )
  ...
  async function handlePress(){
    try{
      setOpen(false)
      console.log(salary)
      if(user.address && age && salary && aged && user.address.length > 10){
        let params={
          id: id,
  ...
  async function handleClick(){
    try{
      ...
      if (res.data.deleteUsers.statusCode === 200){
        setAlert({show:true, title:"success", messageBody:"Deleted Successful!"})
      }
      else{
        setAlert({show:true, title: "error", messageBody:"Invalid Input!"})
      }
      ...
      <Table className={classes.table} aria-label="simple table">
            ...
            {lists.map((list) => (
              ...
              <DialogActions>
                ...
                <Button onClick={() => handlePress(list.EmployeeID)} color="primary">
                  Submit
                </Button>
              </DialogActions>
              </Dialog>
