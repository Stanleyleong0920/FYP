...
export default function EmployeeRequests(){
  ...
  async function fetchApprovedRequests(){
    let res = await API.graphql(
      graphqlOperation(listRequestss, { filter:{ RequesterIdentity:{eq: "Employee" }, Status: {eq:"Request Approved"} }})
    )
  ...
  async function fetchDeclinedRequests(){
    let res = await API.graphql(
      graphqlOperation(listRequestss, { filter:{ RequesterIdentity:{eq: "Employee" }, Status: {eq:"Request Declined"} }})
    )
    if (res.data.listRequestss.items.length > 0){
  ...
  return(
    ...
    View Approved/Declined Requests From Employees
    </Typography>
    {see === true ? <div>
     <br></br>
   <br></br>
