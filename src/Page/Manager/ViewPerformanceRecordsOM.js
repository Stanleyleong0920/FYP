...
export default function ViewPerformanceRecordsOE(){
  ...
  async function fetchData(id) {
    ...
    if (res.data.listUsers.items[0]){
      ...
      fetchUser(res.data.listUsers.items[0].EmployeeID,res.data.listUsers.items[0].Department)
    }
  }

  async function fetchUser(id,department){
    let res = await API.graphql(
      graphqlOperation(listUsers, { filter: { EmployeeID : { ne:id },Identity: { eq: 'Manager' } , Department:{eq:department} }})
    )
  ...
  return(
    ...
    View Other Managers' Performance Records
    </Typography>
