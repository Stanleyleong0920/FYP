...
import { listRequestss } from '../../customQueries'
...
export default function RequestsFromEmployees(props){
  const classes = useStyles();
  const [list,setList] = useState({})
  const [requests, setListss] = useState([])
  ...
  async function fetchData(id) {
    ...
    if (res.data.listUsers.items[0]){
      ...
      fetchEmployeeRequests(res.data.listUsers.items[0].Department)
    }
  }
  
  async function fetchEmployeeRequests(dep){
    let res = await API.graphql(graphqlOperation(listRequestss, { filter: { RequesterIdentity: { eq: 'Employee' }, RequesterDepartment:{eq:dep}, Status:{eq:"Pending for manager to approve"}}}))

    if (res.data.listRequestss.items){
      setListss(res.data.listRequestss.items)
    }
  }
  ...
  async function handleSubmit(status){
    try{
      ...
      if(reason.reason && reason.reason.length > 5){
        if(status === true){
          requestStatus = "Pending for admin to approve" 
        }
        else{
