...
export default function RateEmployees(){
  ...
  async function fetchData(id) {
    ...
    if (res.data.listUsers.items[0]){
      ...
      fetchEmployees(res.data.listUsers.items[0].EmployeeID,res.data.listUsers.items[0].Department)
    }
  }

  async function fetchEmployees(id,dep){
    let res = await API.graphql(
      graphqlOperation(listUsers, { filter: { EmployeeID: {ne: id}, Identity:{eq: "Manager"}, Department: { eq: dep }}})
    )
      ...
      if (review.punctuality && review.responsibility && review.activeness && review.hardworking && review.personality 
        && review.feedback && review.feedback.length > 5){
          ...
          if(res.data.submitRating.statusCode === 200){
            ...
            setAlert({show:true, title:"error", messageBody:"You have rated this manager one week before! Please wait few days/one week to submit again!"})
        ...
        setAlert({show:true, title: "error", messageBody:"Please fThe length of feedback is not enough!"})
        setOpen2(false)
        setOpen(false)
        setReview({
          punctuality: 0,
    ...
    <Table className={classes.table} aria-label="simple table">
          ...
          <TableBody>
            ...
            <TextField
                ...
                fullWidth
            />
            *One week, and each week one time submission, cannot be edited or deleted.
            </DialogContent>
            <DialogActions>
